"use client";
import { CardVolunteer } from "@/components/ui/CardVolunteer";
import MainContainer from "@/components/ui/MainContainer";
import { formatDateRangeID } from "@/lib/formatDateRangeId";
    ...
    <MainContainer>
      <div className="flex flex-col mx-auto text-center gap-6 mt-10 md:mt-40">
        ...
        <p className="md:text-lg max-w-xs text-sm md:max-w-4xl mx-auto">
          Program sukarelawan yang mengajak peserta untuk melakukan aksi nyata
          menjaga dan memulihkan lingkungan melalui kegiatan seperti penanaman
          pohon, bersih-bersih, dan edukasi lingkungan.
        </p>
      ...
      <div className="flex flex-col max-w-5xl mx-auto text-center gap-6 my-10">
        <h1 className="text-4xl font-bold">Yuk, Ikuti!</h1>
        <div className="grid grid-cols-1 mx-auto sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {paginatedItems.map((item) => (
            <CardVolunteer
              key={item.id}
              title={item.title}
              date={formatDateRangeID(item.dateStart, item.dateEnd)}
              location={item.location}
              imageSrc={item.imageUrl}
              href={`/aksi-hijau/volunteer/${item.id}`}
            />
          ))}
        </div>
        <div className="mt-6 flex items-center mx-auto gap-4">
